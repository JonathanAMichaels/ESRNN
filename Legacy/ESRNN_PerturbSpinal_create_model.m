...
interArea = 0.05;
p = [1 interArea 0; interArea 1 interArea; 0 0 1];
...
wFb = zeros(N,F);
if F > 0
    wFb(areaInd{3},:) = randn(length(areaInd{3}),F) / sqrt(length(areaInd{3}));
  %  wFb = wFb * 1e-1;
end

%% Initialize output weights
wOut = zeros(B,N);
wOut(:,areaInd{3}) = randn(B,length(areaInd{3})) / sqrt(length(areaInd{3}));
%wOut = wOut * 1e-2;
