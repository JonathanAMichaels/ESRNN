...
preTime = 40;
moveTime = 30;
totalTime = preTime+moveTime; % Total trial time
...
targetFunPassthrough.kinTimes = preTime/2:totalTime;
...
for cond = 1:numConds
    ...
    targ{cond} = [nan(2, totalTime-1), [sin(ang(cond)); cos(ang(cond))]];
    inp{cond} = [ones(1,totalTime)*sin(ang(cond)); ones(1,totalTime)*cos(ang(cond)); [ones(1,preTime), zeros(1,moveTime)]];
end
...
fitnessFun = @ESRNN_COHoldFitnessFun;
...
batchSize = 1;
learningRate = 1e-1;
optimizerParams = [0.1, 0.9, 0.999, 1e-1]; %1e-2
