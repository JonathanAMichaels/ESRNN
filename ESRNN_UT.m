...
try
    if ispc
        baseDir = 'E:\jmichaels\Projects\UT';
    else
       baseDir = '/Users/jonathanamichaels/Desktop/UT';
       % baseDir = '/Users/jonathanamichaels/Desktop/jmichaels/Projects/UT';
    end
    % Select base directory
    cd(baseDir)
    disp('Running local config')
catch
    baseDir = '/tmp/jmichaels/Projects/UT';
    local = false;
    evalOpts = [0 1];
...
inpFun = @ESRNN_UT_inpFun;
targetFun = @ESRNN_UT_TargetFun; % handle of custom target function
plotFun = @ESRNN_Perturb_PlotFun;
policyInitFun = @ESRNN_Perturb_create_model;
fitnessFun = @ESRNN_UT_FitnessFun;
...
for reps = 1
    ...
    [inp, fitnessFunInputs, targetFunPassthrough] = ESRNN_UT_inpFun;
    % run model
...
if strcmp(func2str(policyInitFun), 'ESRNN_Perturb_create_model')
    ...
    areaInd{3} = round(N*0.90)+1 : round(N*0.95);
...
for cond = 1:length(inp)
    ...
    bigZ0{cond} = Z0{cond}(:,end-offset:end);%kin(cond).FOut(end-129:end,:)';
  %  vel(:,cond,:) = diff(Z1{cond}(:,end-offset:end),1,2)';
    ...
    combinedParams = {{1, [1 3]}, {2, [2 3]}, {3}, {[1 2], [1 2 3]}};
        ...
        time = 1:size(firingRatesAverage,4);
            ...
            optimalLambda = dpca_optimizeLambda(firingRatesAverage, firingRates, trialNum, ...
                ...
                'numRep', lambdaReps); % increase this number to ~10 for better accuracy
    
        
           
        [W,V,whichMarg] = dpca(firingRatesAverage, 6, ...
            'combinedParams', combinedParams, ...
