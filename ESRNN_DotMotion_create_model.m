...
wIn = zeros(N,I);
wIn(areaInd{1},1) = randn(length(areaInd{1}),1) / sqrt(length(areaInd{1}));
wIn(areaInd{2},2) = randn(length(areaInd{2}),1) / sqrt(length(areaInd{2}));
...
wOut = zeros(B,N);
wOut(:,areaInd{2}) = randn(B,length(areaInd{2})) / sqrt(length(areaInd{2}));
wOut = wOut * 1e-9;
