...
inpFun = @ESRNN_Perturb_inpFun_CO;
targetFun = @ESRNN_Perturb_TargetFun; % handle of custom target function
...
for reps = 1:10
    [inp, fitnessFunInputs, targetFunPassthrough] = ESRNN_Perturb_inpFun_CO;
    % run model
...
N = net.N;
clear areaInd
if strcmp(func2str(policyInitFun), 'ESRNN_create_model')
    areaInd{1} = 1:N;
else
...
for cond = 1:length(inp)
    bigR{cond} = R{cond}(:,:);
    bigZ{cond} = Z1{cond}(:,:);
    bigZ0{cond} = Z0{cond}(:,:);%kin(cond).FOut(end-129:end,:)';
    vel(:,cond,:) = diff(Z1{cond}(:,:),1,2)';
    useTarg{cond} = fitnessFunInputs{cond}(:,:);
